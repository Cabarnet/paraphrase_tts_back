...
Последним звеном цепочки идёт **Groq** — отдельный провайдер с собственным пулом
квот (бесплатно 1000 запросов и 200 000 токенов в сутки, карта не нужна). Он
подключается автоматически, если задана переменная `GROQ_API_KEY`, и подхватывает
запросы, когда дневная квота Google исчерпана. Модель задаётся в `GROQ_MODEL`
(по умолчанию `openai/gpt-oss-120b`).
